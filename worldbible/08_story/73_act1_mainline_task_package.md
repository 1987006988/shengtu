# Act I 主线任务层包

## 1. 文件定位

本文件以 [72_act1_mainline_prose_review.md](/E:/python_learn/shengtu/worldbible/08_story/72_act1_mainline_prose_review.md:1) 的 `PASS` 为唯一前置门槛。  
没有这个 `PASS`，本文件不应被打开。

本文件只服务：

- `Act I / A1`
- `CH01–CH03`
- 主线任务层

本文件明确不服务：

- 支线任务层
- `CH04+`
- 旧 narrative-lab 回收
- 全量 `MQ / SQ / Journal / Codex / Item / System feedback`

## 2. 上游继承边界

本任务层包只允许继承：

- `worldbible/04_main_world/15_bailuyuan_region_bible.md`
- `worldbible/04_main_world/16_book1_binding_boundary.md`
- `worldbible/07_characters/21_party_story_function_bible.md`
- `worldbible/07_characters/22_character_progression_master.md`
- `worldbible/08_story/54_book1_chapter_blueprint.md`
- `worldbible/08_story/55_book1_scene_blueprint.md`
- `worldbible/08_story/56_book1_reveal_foreshadow_table.md`
- `worldbible/08_story/57_book1_character_progression_table.md`
- `worldbible/08_story/58_book1_world_state_table.md`
- `worldbible/08_story/66_act1_mainline_prose_ch01.md`
- `worldbible/08_story/67_act1_mainline_prose_ch02.md`
- `worldbible/08_story/68_act1_mainline_prose_ch03.md`
- `worldbible/08_story/72_act1_mainline_prose_review.md`

## 3. 当前正式输出文件

- `worldbible/08_story/74_act1_mainline_mq.md`
- `worldbible/08_story/75_act1_mainline_journal.md`
- `worldbible/08_story/76_act1_mainline_codex.md`
- `worldbible/08_story/77_act1_mainline_state_trigger_table.md`
- `worldbible/08_story/78_act1_mainline_task_review.md`

## 4. 主线任务层硬规则

1. 只允许把 `A1 / CH01–CH03` 已存在的正文与蓝图翻译成任务层，不得新增并列主线。
2. 任何 `MQ / Journal / Codex / State Trigger` 都必须先服务场景行动判断，再服务信息释放。
3. 任何条目若需要 `CH04+` 的路权政治、祁渡川接口或灰线历史线才能成立，视为越界。
4. 任何条目若需要旧 `33a–43a` 或旧 `game_narrative_*` 才能补齐，视为越界。

## 5. 当前禁止事项

1. 不得趁机重开全量 `MQ / SQ / Journal / Codex / Item / System feedback`。
2. 不得把 `A1` 主线任务层写成 `Act I` 全段任务层已开放。
3. 不得把任务层反向改写 `66–68` 的正文结论。

## 6. 一句话口径

> `73` 的作用不是开全书任务层，而是只把 `A1 / CH01–CH03` 的主线任务层压成一套可审最小闭环。
